...
test(pt15) :-
    interval(pt(-0.5, 5, 0), Res),
    equal(Res, 0.3191...0.3192).

test(pt16) :-
    interval(pt(-0.5, 5, 1), Res),
    equal(Res, 0.0723...0.0724).

test(pt17) :-
    interval(pt(-0.5... -0.2, 5, 0), Res),
    equal(Res, 0.3191...0.4247).

test(pt18) :-
    interval(pt(-0.5... -0.2, 5, 1), Res),
    equal(Res, 0.0723...0.1175).

test(pt19) :-
    interval(pt(-0.5, 5...6, 0), Res),
    equal(Res, 0.3174...0.3192).

test(pt20) :-
    interval(pt(-0.5, 5...6, 1), Res),
    equal(Res, 0.0714...0.0724).

test(pt21) :-
    interval(pt(-0.5, 5, 1...2), Res),
    equal(Res, 0.0071...0.0724).

test(pt22) :-
    interval(pt(-0.5... -0.2, 5...6, 0), Res),
    equal(Res, 0.3174...0.4247).

test(pt23) :-
    interval(pt(-0.5... -0.2, 5...6, 1), Res),
    equal(Res, 0.0714...0.1175).

test(pt24) :-
    interval(pt(-0.5...0.2, 5, 1...2), Res),
    equal(Res, 0.0071...0.2095).
...
test(pt46) :-
    interval(pt(-0.5, 5, 0, true), Res),
    equal(Res, 0.3191...0.3192).

test(pt47) :-
    interval(pt(-0.5, 5, 1, false), Res),
    equal(Res, 0.9276...0.9277).

test(pt48) :-
    interval(pt(-0.5... -0.2, 5, 0, true), Res),
    equal(Res, 0.3191...0.4247).

test(pt49) :-
    interval(pt(-0.5... -0.2, 5, 1, false), Res),
    equal(Res, 0.8825...0.9277).

test(pt50) :-
    interval(pt(-0.5, 5...6, 0, true), Res),
    equal(Res, 0.3174...0.3192).

test(pt51) :-
    interval(pt(-0.5, 5...6, 1, false), Res),
    equal(Res, 0.9276...0.9286).

test(pt52) :-
    interval(pt(-0.5, 5, 1...2, true), Res),
    equal(Res, 0.0071...0.0724).

test(pt53) :-
    interval(pt(-0.5, 5, 1...2, false), Res),
    equal(Res, 0.9276...0.9929).

test(pt54) :-
    interval(pt(-0.5... -0.2, 5...6, 0, true), Res),
    equal(Res, 0.3174...0.4247).

test(pt55) :-
    interval(pt(-0.5... -0.2, 5...6, 1, false), Res),
    equal(Res, 0.8825...0.9286).

test(pt56) :-
    interval(pt(-0.5...0.2, 5, 1...2, true), Res),
    equal(Res, 0.0071...0.2095).

test(pt57) :-
    interval(pt(-0.5...0.2, 5, 1...2, false), Res),
    equal(Res, 0.7905...0.9929).
...
test(pt60) :-
    interval(pt(-0.5, 5, 1, true, false), Res),
    equal(Res, 0.0723...0.0724).

test(pt61) :-
    interval(pt(-0.5... -0.2, 5, 1, false, true), Res),
    equal(Res, -0.1249... -0.0750).

test(pt62) :-
    interval(pt(-0.5, 5...6, 1, true, true), Res),
    equal(Res, -2.6393... -2.6269).

test(pt63) :-
    interval(pt(-0.5... -0.2, 5...6, 1, false, false), Res),
    equal(Res, 0.8825...0.9286).

test(pt64) :-
    interval(pt(-0.5, 5...6, 1...2, true, false), Res),
    equal(Res, 0.0070...0.0724).

test(pt65) :-
    interval(pt(-0.5...0.2, 2...5, 1...2, true, true), Res),
    equal(Res, -4.9362... -1.5630).
...
test(qt64) :-
    interval(qt(0.4, 5, 0, true), Res),
    equal(Res, -0.2672... -0.2671).

test(qt65) :-
    interval(qt(0.4, 5, 0, false), Res),
    equal(Res, 0.2671...0.2672).

test(qt66) :-
    interval(qt(0.4, 5, 1, true), Res),
    equal(Res, 0.7779...0.7780).

test(qt67) :-
    interval(qt(0.4, 5, 1, false), Res),
    equal(Res, 1.3424...1.3425).

test(qt68) :-
    interval(qt(0.6...0.7, 5, 0, true), Res),
    equal(Res, 0.2671... 0.5595).

test(qt69) :-
    interval(qt(0.6...0.7, 5, 0, false), Res),
    equal(Res, -0.5595... -0.2671).

test(qt70) :-
    interval(qt(0.6...0.7, 5, 1, true), Res),
    equal(Res, 1.3424...1.6759).

test(qt71) :-
    interval(qt(0.6...0.7, 5, 1, false), Res),
    equal(Res, 0.4936...0.7780).

test(qt72) :-
    interval(qt(0.4, 5...6, 0, true), Res),
    equal(Res, -0.2672... -0.2648).

test(qt73) :-
    interval(qt(0.4, 5...6, 0, false), Res),
    equal(Res, 0.2648...0.2672).

test(qt74) :-
    interval(qt(0.4, 5...6, 1, true), Res),
    equal(Res, 0.7725...0.7780).

test(qt75) :-
    interval(qt(0.4, 5...6, 1, false), Res),
    equal(Res, 1.3267...1.3425).

test(qt76) :-
    interval(qt(0.4, 5, 1...2, true), Res),
    equal(Res, 0.7779...1.8051).

test(qt77) :-
    interval(qt(0.4, 5, 1...2, false), Res),
    equal(Res, 1.3424...2.4516).

test(qt78) :-
    interval(qt(0.4...0.7, 2...5, 0, true), Res),
    equal(Res, -0.2887...0.6173).

test(qt79) :-
    interval(qt(0.4...0.7, 2...5, 0, false), Res),
    equal(Res, -0.6173...0.2887).

test(qt80) :-
    interval(qt(0.4...0.7, 2...5, 1, true), Res),
    equal(Res, 0.7779...1.9633).

test(qt81) :-
    interval(qt(0.4...0.7, 2...5, 1, false), Res),
    equal(Res, 0.4936...1.5021).

test(qt82) :-
    interval(qt(0.4...0.7, 5, 1...2, true), Res),
    equal(Res, 0.7779...2.8529).

test(qt83) :-
    interval(qt(0.4...0.7, 5, 1...2, false), Res),
    equal(Res, 0.4936...2.4516).

test(qt84) :-
    interval(qt(0.4, 5...6, 1...2, true), Res),
    equal(Res, 0.7725...1.8051).

test(qt85) :-
    interval(qt(0.4, 5...6, 1...2, false), Res),
